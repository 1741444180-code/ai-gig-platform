...
class TestListDemands:
    ...
    @pytest.mark.asyncio
    async def test_list_demands_empty(self, client: AsyncClient):
    ...
    async def test_list_demands_pagination(self, client: AsyncClient, test_user: User):
        ...
        resp = await client.get("/api/v1/demands?page=1&page_size=10")
        assert resp.status_code == 200
        data = resp.json()
        assert "items" in data
    ...
    @pytest.mark.asyncio
    async def test_list_demands_category_filter(self, client: AsyncClient):
        """Category filter should work."""
        resp = await client.get("/api/v1/demands?category=文案")
        assert resp.status_code == 200

    @pytest.mark.asyncio
    async def test_list_demands_status_filter(self, client: AsyncClient):
        """Status filter should work."""
        resp = await client.get("/api/v1/demands?status=open")
        assert resp.status_code == 200
...
class TestGetDemandDetail:
    ...
    async def test_get_demand_exists(self, client: AsyncClient, test_user: User):
        ...
        resp = await client.get(f"/api/v1/demands/{demand_id}")
        assert resp.status_code == 200
        data = resp.json()
    ...
    @pytest.mark.asyncio
    async def test_get_demand_not_found(self, client: AsyncClient):
        """Get non-existing demand should return 404."""
        resp = await client.get("/api/v1/demands/non-existing-id")
